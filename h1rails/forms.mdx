...
Implement clean selects with type to search, multi select and more, by wrapping a normal select in a `<better-select>` tag&#x20;
...
- Read our [article](/h1rails/form_patterns) which covers multi step forms.
- Check out our [Multi Step Form demo](https://demo.h1rails.com/demos)

### Nested Forms

Sometimes we want to build experiences where users can create or edit a list of options in a single form. Rails nested relationships handle this for us. Read [this article](/h1rails/form_patterns#nested-relationships) for full details
